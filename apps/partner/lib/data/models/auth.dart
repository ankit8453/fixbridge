...
class AuthUser {
  ...
  ///
  /// Exists because the customer's name is stored on the customer profile,
  /// not on the user row, so saving it cannot be reflected by re-reading
  /// `/auth/me` — the two are different columns.
  AuthUser withName(String? newName) => AuthUser(
        id: id,
        phone: phone,
        name: newName,
        roles: roles,
