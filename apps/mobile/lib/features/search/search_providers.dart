...
///
/// A thin alias over [resolvedLocationProvider] so the home header and the
/// search results can never disagree about where "near me" is — two
/// independent resolvers would eventually drift, and a header saying "Home"
/// above results measured from the city centre is worse than either alone.
final searchOriginProvider =
    FutureProvider<({double lat, double lng, bool usingFallback})>((ref) async {
  final loc = await ref.watch(resolvedLocationProvider.future);
  return (lat: loc.lat, lng: loc.lng, usingFallback: loc.isGuess);
});
