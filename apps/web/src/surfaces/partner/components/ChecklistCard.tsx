...
export function ChecklistCard({ completeness }: { completeness: ProviderCompletenessResponse }) {
  ...
  return (
    <Card
      ...
      actions={
        <Badge tone={completeness.isListed ? 'success' : 'warning'}>
      ...
      <ul className="flex flex-col gap-2">
        {sorted.map((entry) => (
          <li key={entry.item} className="flex items-center gap-3">
            ...
            <span
              className={`flex-1 text-sm ${entry.satisfied ? 'text-muted line-through' : 'text-slate-800'}`}
            >
              {t(`partner.checklist.item.${entry.item}`)}
            </span>
            {entry.required ? (
              <span className="text-xs text-muted">{t('partner.checklist.required')}</span>
            ) : null}
          </li>
        ))}
      </ul>
