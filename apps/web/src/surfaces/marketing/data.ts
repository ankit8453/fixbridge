...
/**
 * Public data access for the marketing surface, ported from
 * `legacy-next-src/components/marketing/data.ts` — same endpoints, same
 * "never fabricate a number" rule, adapted from Next's server-only fetch to
 * a plain async function any component can call from a `useQuery`. Every
 * function here calls the same public, unauthenticated endpoints a
 * signed-out visitor's browser could call directly (`skipAuth: true`); a
 * function that cannot reach the API returns null instead of throwing, so a
 * marketing page renders without a stat rather than crashing (PHASE12_PROMPT.md:
 * "do NOT invent an endpoint or hardcode a number — render the page without
 * that stat and report it").
 */
 ...
 *
 * The `/services/:slug` route and the slug lookup both want this shape —
 * clusters get their own landing page too (a cluster like "Electrical" is
 * the actual "electrician in Jabalpur" search target; the leaves underneath
 * it, like "House wiring & repair", are the more specific long-tail ones).
 * `/search/providers?category_id=` already treats a cluster id as "every
 * service beneath it" (docs/API.md), so one detail page template serves
 * both levels with no special-casing.
 */
 ...
 * providers) — generous for the pilot's seed data, and bounded so a homepage
 * load never turns into an unbounded burst against the search endpoint's own
