...
/**
 * Base location + service radius — two of the five hard-gate items in one
 ...
 * (PHASE12_PROMPT.md §C.1: "base location (geolocation)") — no map picker,
 * same "map-less v1, honest about it" call the customer surface's address
 * book makes. A technician on a cheap Android phone almost certainly has
 * GPS; typing lat/lng by hand is the fallback for the browsers/permissions
 * that refuse it, not the primary path.
 */
export function LocationForm({ profile }: { profile: ProviderProfileResponse }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {geoError ? <p className="text-sm font-medium text-red-700">{geoError}</p> : null}
      ...
      {lat && lng ? (
        ...
        <p className="text-sm text-slate-500">{t('partner.location.notSet')}</p>
      )}
        ...
        <ErrorState
          error={
            save.error instanceof ApiError
              ? save.error
              : new ApiError(0, 'UNKNOWN', String(save.error), null)
          }
          onRetry={() => save.reset()}
        />
      ) : null}
