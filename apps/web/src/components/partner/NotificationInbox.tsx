...
const CRITICALITY_TONE = { critical: 'bad', normal: 'info', low: 'neutral' } as const;

/**
 * A plain notification inbox. `src/components/customer/**` does not exist
 * yet in this working tree (surface B is still placeholder-only — see
 * `src/app/[locale]/app/page.tsx`), so per the phase brief this is a
 * partner-owned component rather than an import. If/when the customer
 * surface adds an equivalent, the two should be reconciled into one shared
 * component under a location neither surface's lane currently covers.
 */
