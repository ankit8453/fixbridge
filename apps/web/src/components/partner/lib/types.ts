/**
 * Wire types for surface C, hand-written rather than imported.
 *
 * `packages/shared` only carries types every phase needs (auth, money,
 * geography) — domain DTOs like a provider profile or a booking detail live
 * only in `apps/api/src/modules/**` and are not published anywhere a Next.js
 * app can import from. These mirror the API's `types.ts` files field for
 * field (cited per block) so a shape drift shows up as a TypeScript error at
 * the call site instead of a silent `undefined` in production.
 */
