    ...
    <div className="flex flex-col items-center gap-4">
      <div className="flex gap-3" aria-label={t('partner.otp.enteredLabel')}>
        {Array.from({ length }).map((_, index) => (
          <div
            key={index}
            className="flex h-14 w-11 items-center justify-center rounded-lg border-2 border-slate-300 text-2xl font-bold tabular-nums text-slate-900"
          >
            {digits[index] ?? ''}
          </div>
        ))}
      ...
      {error ? (
        <p role="alert" className="text-center text-base font-semibold text-red-700">
          {error}
      ...
      {pending ? <p className="text-sm text-slate-500">{t('partner.otp.checking')}</p> : null}
      ...
      <div className="grid grid-cols-3 gap-3">
        {KEYS.map((key) => (
          <button
            ...
            className="flex h-16 w-20 items-center justify-center rounded-xl border border-slate-300 bg-white text-2xl font-semibold text-slate-900 active:bg-slate-100 disabled:opacity-50"
          >
