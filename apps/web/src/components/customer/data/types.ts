/**
 * Response/request shapes for surface B, copied field-for-field from the API
 * route handlers this app calls (`apps/api/src/modules/*\/types.ts` and
 * `docs/API.md`) rather than guessed. Kept local to `components/customer`
 * (not `packages/shared`, which is deliberately small — see its own header
 * comment) because these are wire shapes one surface consumes, not a type
 * two runtimes need to agree on the way `AuthUser` does.
 */
...
export interface CategoryNode {
   ...
   * the same gates search applies; a cluster sums its services). Present in
   * the real response (`apps/api/src/modules/categories/types.ts`) though the
   * `docs/API.md` example omits it — trusting the route source over the doc.
   * Cached for 5 minutes server-side: a browsing hint, not a live number.
   */
