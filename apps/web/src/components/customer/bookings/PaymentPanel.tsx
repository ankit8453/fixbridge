...
import {
  ...
  useRazorpayReady,
  useStartPayment,
} from '@/components/customer/data/payments';
import { Button, ErrorState, QueryState } from '@/components/ui';
import type { PayableView } from '@/components/customer/data/types';
 ...
 * surface's locale files must nest everything under a single top-level
 * `app` key (see the phase brief), so the server key is remapped onto this
 * app's own `app.payable.*` copy rather than looked up directly, which
 * would silently fall through to showing the raw key string.
 */
