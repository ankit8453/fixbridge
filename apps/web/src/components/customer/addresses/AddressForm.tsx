...
import { getCurrentLocation, GeoError, type GeoErrorReason } from '@/components/customer/data/geo';
import { useCreateAddress, useUpdateAddress } from '@/components/customer/data/addresses';
import { Button, ErrorState, Field, Modal, Select, TextArea, TextInput } from '@/components/ui';
import type { AddressResponse, AddressLabel } from '@/components/customer/data/types';

/**
 * Create/edit, landmark-first — the tier-2 pattern the phase brief calls
 * out: a street address alone rarely gets a technician to the right gate,
 * a landmark does. Coordinates come **only** from browser geolocation in
 * this v1 (no map picker, no geocode-as-you-type — "map-less v1" per the
 * phase spec) or, if the customer never grants location, the API's own
