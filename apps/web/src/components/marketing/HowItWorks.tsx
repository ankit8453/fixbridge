 ...
 * (`marketing.howItWorks.s1..s4`); the dedicated page just wraps them in more
 * explanation. One copy of the steps to keep, not two that can drift.
 *
 * No outer `max-w`/`mx-auto` wrapper on purpose — both call sites already
 * provide their own container, and nesting two centred max-widths would
 * double the horizontal padding.
 */
export function HowItWorksSteps({ locale }: { locale: Locale }) {
  const t = getT(locale);
