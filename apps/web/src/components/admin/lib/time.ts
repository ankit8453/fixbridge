/**
 * Timestamps as ops read them. Ported verbatim from apps/admin/src/lib/time.ts
 * — no react-router or Next dependency in the original, so nothing here
 * changed by moving into Next.js.
 *
 * Relative on the surface ("3h ago") because the only question on a queue
 * screen is "how long has this person been waiting". The absolute value
 * always follows in a `title` attribute — the moment a dispute starts, "3h
 * ago" is worthless and the exact instant is the whole answer.
 */
...
/** The full instant, in IST — the timezone every one of these events actually happened in. */
