/**
 * The API's response shapes, as far as this surface reads them.
 *
 * Ported from apps/admin/src/api/types.ts (Phase 11) with `SessionUser` and
 * `AuthSession` dropped — this surface authenticates through the web app's
 * shared session (`@/lib/auth/useAuth`), not a console-specific login, so
 * those two never existed here. `Badge` was renamed `TrustBadge`: this file
 * sits inside `components/admin`, next to the shared `Badge` UI component
 * (`@/components/ui`), and a type and a component sharing one identifier one
 * import apart is a footgun for the next person who edits this file, not a
 * risk worth keeping for fidelity to the original name.
 *
 * Deliberately partial otherwise. These are hand-written mirrors of what
...
/**
 * Phase 12 correction: the audit log endpoint now returns which slice of the
 * table the caller is looking at — `'own'` for an ops token (the server
 * forces that filter, it is not a client choice), `'all'` for admin. The
 * audit page renders this so an ops user reading their own actions never
 * mistakes the list for the whole log.
 */
