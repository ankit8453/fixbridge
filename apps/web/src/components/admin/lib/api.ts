 ...
 * apps/admin/src/api/endpoints.ts. Two differences from the Phase 11
 * original:
 *
 * 1. No `requestOtp`/`verifyOtp`/`logoutSession`. This surface authenticates
 *    through the web app's one shared login (`@/lib/auth/session`), not a
 *    console-specific OTP form — see PHASE12_PROMPT.md §D ("role in the
 *    token decides landing surface").
 * 2. Every call goes through `adminRequest`, which pins `Accept-Language` to
 *    `en` regardless of the page's `[locale]` segment. Admin stays
 *    English-only exactly as apps/admin/README.md decided ("if ops hiring
 *    ever makes Hindi the working language of the room, the change is that
 *    header plus a translation layer, not a rewrite") — that decision is
 *    still deferred, not revisited, just relocated from a standalone fetch
 *    wrapper into one option on the shared client.
 *
 * The API's list endpoints still disagree about pagination parameter names
 * (`pageSize` on Phase 4's verification queue, `page_size` everywhere else,
 ...
 * all (no city-config screen was ported from Phase 11; none existed). This
...
/** Ops-accessible: "payout batch create/review" is judgment work per the Phase 12 split. */
...
/**
 * Phase 12 correction: the response now also carries `scope` — `'own'` when
 * the server forced the filter to this actor (an ops token) or `'all'` for
 * admin. `parsePage` only normalises the paginated shape, so `scope` is read
 * off the raw body directly, defaulting to `'own'` — the safer misread if a
 * response somehow omitted it is "you're seeing less than you think", not
 * the other way round.
 */
