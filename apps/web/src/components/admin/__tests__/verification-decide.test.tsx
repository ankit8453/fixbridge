...
import VerificationCasePage from '@/app/[locale]/admin/verification/[caseId]/page';
import { mockApi, renderAdminPage, waitForCall } from './harness';

/**
 * Ported from apps/admin/src/test/verification-decide.test.tsx. The
 * component under test and every assertion are unchanged; only the routing
 * plumbing differs — `useParams()` now comes from `next/navigation` rather
 * than a mounted `MemoryRouter`, so it is mocked directly below instead of
 * being supplied via a real route.
 */
vi.mock('next/navigation', () => ({
  useParams: () => ({ locale: 'en', caseId: 'case-1' }),
  usePathname: () => '/en/admin/verification/case-1',
...
describe('deciding a verification case', () => {
  it('sends the decision and the notes to the decide endpoint', async () => {
    ...
    renderAdminPage(<VerificationCasePage />);
