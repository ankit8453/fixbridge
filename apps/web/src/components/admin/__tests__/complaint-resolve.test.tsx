...
import ComplaintDetailPage from '@/app/[locale]/admin/complaints/[complaintId]/page';
import { mockApi, renderAdminPage, waitForCall } from './harness';

/** Ported from apps/admin/src/test/complaint-resolve.test.tsx, unchanged. */
vi.mock('next/navigation', () => ({
  useParams: () => ({ locale: 'en', complaintId: 'complaint-1' }),
  usePathname: () => '/en/admin/complaints/complaint-1',
...
describe('resolving a complaint', () => {
  it('requires both a severity and a note, then sends them together', async () => {
    ...
    renderAdminPage(<ComplaintDetailPage />);
