...
import { CashCollectButton } from '@/components/partner/CashCollectButton';
import { OtpKeypad } from '@/components/partner/OtpKeypad';
import { QuoteBuilder } from '@/components/partner/QuoteBuilder';
import { ReasonPicker } from '@/components/partner/ReasonPicker';
import {
  acceptBooking,
  cancelBooking,
  fetchBooking,
 ...
 * are not under this surface's `partner.*` namespace, and `src/locales/{hi,en}.json`
 * are outside this agent's lane (see PHASE12_PROMPT.md "Files you own"). Rather
 * than reach into a catalog this surface does not control, the three known
...
export default function JobDetailPage() {
  ...
  const bookingKey = partnerKeys.booking(bookingId);
  const bookingQuery = useQuery({ queryKey: bookingKey, queryFn: () => fetchBooking(bookingId) });
  ...
  const reject = useMutation({
    mutationFn: ({ reason, note }: { reason: string; note?: string }) =>
      rejectBooking(bookingId, reason, note),
    onSuccess: () => {
      invalidateBooking();
      setShowReject(false);
    },
  ...
  const enRoute = useMutation({
    mutationFn: () => markEnRoute(bookingId),
    onSuccess: invalidateBooking,
  });
  ...
  const startOtp = useMutation({
    mutationFn: (otp: string) => submitStartOtp(bookingId, otp),
    onSuccess: invalidateBooking,
  });
  const endOtp = useMutation({
    mutationFn: (otp: string) => submitEndOtp(bookingId, otp),
    onSuccess: invalidateBooking,
  });
  ...
  const paymentsQuery = useQuery({
    queryKey: ['partner', 'payments', bookingId],
                ...
                <div className="mt-2">
                  <OtpKeypad
                    ...
                    error={
                      ...
                      (startOtp.isError ? (startOtp.error as ApiError).message : null)
                    }
              ...
              <>
                <Card title={t('partner.job.quotesTitle')}>
                  {booking.quotations.length === 0 ? (
                    <p className="text-sm text-slate-500">{t('partner.job.noQuotesYet')}</p>
                  ) : (
                ...
                {!booking.approvedQuotation ? (
                  <Card title={t('partner.job.newQuoteTitle')}>
                    <QuoteBuilder bookingId={bookingId} onSent={invalidateBooking} />
                  </Card>
                ) : null}
                  ...
                  <div className="mt-2">
                    <OtpKeypad
                      ...
                      error={
                        ...
                        (endOtp.isError ? (endOtp.error as ApiError).message : null)
                      }
              ...
              <Card title={t('partner.job.payableTitle')}>
                ...
                >
                  {({ payments }) => {
                    ...
                    return (
                      <CashCollectButton bookingId={bookingId} amountPaise={booking.payablePaise} />
                    );
                  }}
                </QueryState>
              </Card>
