...
import type { Payout } from '@/components/admin/lib/types';
...
import { Button, Card, DetailRow, QueryState, Table, Td, Th, Tr } from '@/components/ui';
 ...
 * from apps/admin/src/pages/PayoutBatchPage.tsx.
 *
 ...
 *
 * **Phase 12 permission split:** marking a line paid or failed is
 * admin-only (`ADMIN_ONLY_ROUTES` in apps/api/src/core/audit.ts lists both
                    ...
                    <Tr key={payout.id}>
                      ...
                      <Td>
                        {/* Hidden, not disabled, for ops: these two calls are
                            admin-only server-side, and a button that always
                            403s for a whole role teaches nothing except that
                            the console tried and failed on their behalf. */}
                        {payout.status !== 'paid' && canRecordPayouts ? (
