...
import { fetchReviewReports, hideReview } from '@/components/admin/lib/api';
import { useAdminMutation } from '@/components/admin/lib/mutations';
import { useFilters } from '@/components/admin/lib/filters';
import type { ReviewReport } from '@/components/admin/lib/types';
...
import { Badge, Button, Card, Pagination, QueryState, Spinner } from '@/components/ui';

/**
 * Reported reviews, with enough context to judge one. Ported from
 * apps/admin/src/pages/ReviewsPage.tsx.
 *
...
export default function ReviewsPage() {
  return (
    <Suspense fallback={<Spinner label="Loading reported reviews…" />}>
      <ReviewsContent />
    </Suspense>
  );
}
      ...
      <dl className="mt-2 space-y-0.5 text-xs text-slate-600">
        ...
        {review?.subjectUserId ? (
          <div>
            ...
            <AdminLink
              className="text-blue-700 hover:underline"
              href={`/providers/${review.subjectUserId}`}
            >
              {review.subjectUserId}
            </AdminLink>
          </div>
