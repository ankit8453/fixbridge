...
import type { BookingTimelineResponse } from '../api/types';
 ...
 *
 * This is the dispute screen. When a customer says they were overcharged and a
 * technician says they were not, four separate record types settle it and they
 * only settle it *interleaved*: the event log, the quotation versions, the
 * payments, and what each side was actually told. Four side-by-side tables would
 * make the reader do the merge in their head, and the whole question is what
 * happened before what.
 *
 * The actor label matters as much as the ordering — "cancelled" answers nothing;
 * "cancelled by the technician" is the answer.
 */
...
export function BookingTimeline({ data }: { data: BookingTimelineResponse }) {
  ...
  if (entries.length === 0) {
    return <p className="text-sm text-slate-500">Nothing has happened on this booking yet.</p>;
  }
