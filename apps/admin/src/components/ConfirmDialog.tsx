...
/**
 * The confirmation dialog, with the reason field inside it.
 *
 * Two separate rules meet here, and they are the reason this is one component
 * rather than a `window.confirm`:
 *
 *   1. **Every destructive or money action is confirmed.** Suspending somebody,
 *      cancelling their job, moving a payout — all of it is one click away from
 *      a list, and a misclick on a list is a normal human event.
 ...
 *      because the note is what makes the audit row worth reading six months
 *      later. Asking for it in a second step would make "confirm" the decision
 *      and the note an afterthought — the exact inversion the audit trail exists
 *      to prevent.
 *
 * Client-side validation is a courtesy, not a control: it saves a round trip and
 * names the field, but the server's `min(3)` is the actual rule.
 */
...
export function ConfirmDialog({
  ...
  /**
   * State is seeded once, on mount, and every call site mounts this component
   * only while its dialog is open. That is what guarantees a fresh dialog per
   * decision — a form still holding the previous row's reason is how the wrong
   * technician gets suspended.
   */
...
/** The reason/note field every mutation in this console needs. */
