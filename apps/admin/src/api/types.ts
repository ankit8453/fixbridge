/**
 * The API's response shapes, as far as this console reads them.
 *
 * Deliberately partial. These are hand-written mirrors of what
 * `apps/api/src/modules/admin` returns, not generated types, so every field here
 * is one a screen actually renders — and anything the API adds later is simply
 * ignored rather than breaking a build. Optional markers are used liberally on
 * nested objects because several of these endpoints return relations that can
 * genuinely be absent (a provider with no verification row, a booking with no
 * quotation), and a console that throws on a missing relation is a console that
 * dies on exactly the edge case ops opened it to look at.
 */
 ...
 *
 * The console shows the trust score's **inputs** rather than a weighted
 * breakdown, because these columns are what the admin aggregate returns and they
 * are what an ops user needs when a technician asks why their score dropped: the
 * ratings, the acceptance record, the cancellations and the upheld complaints.
 * Weights live in API config and are not exposed on this endpoint.
 */
