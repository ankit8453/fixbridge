...
import type { VerificationCaseDetail, VerificationDocument } from '../api/types';
import { ConfirmDialog } from '../components/ConfirmDialog';
import { PageHeader } from '../components/Layout';
import { Timestamp } from '../components/Timestamp';
import { BadgeLevel, StatusBadge } from '../components/ui/Badge';
...
export function VerificationCasePage() {
  ...
  const invalidate = [['verification'], ['summary']];
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Link className="text-sm text-blue-700 hover:underline" to="/verification">
            ← Back to the queue
          </Link>
        }
      />
    ...
    >
      <dl>
        ...
        <DetailRow label="Technician">
          <Link className="text-blue-700 hover:underline" to={`/providers/${detail.provider.id}`}>
 ...
 * cannot do it across two browser tabs. Everything else gets a plain link and
 * nothing more — no `download` attribute, no auto-open, no embedding a PDF or
 * an unknown MIME type in a frame. The storage layer already signs these URLs to
 * be served inert, and the console should not undo that by being helpful.
 */
 ...
 *
 * Verification history refuses UPDATE at the database level, so this list is the
 * case — its status is only a projection of it. Payloads are printed raw because
 * a reviewer re-reading a decision needs what was actually submitted, not a
